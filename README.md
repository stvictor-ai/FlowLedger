...
- 记录向投资账户的每一笔**入金 / 出金**
- 支持多资产类型：大 A、美股、港股、加密货币、基金、黄金、期货、其他
...
- 导入前预览：自动识别新增、完全重复、疑似重复、无效行，安全合并
- 导入历史：保留最近 20 次导入批次，可一键撤销
- 自动快照（最多 10 份 FIFO）+ 手动快照 + 一键恢复
...
下载 `index.html`，用任意浏览器打开即可。无需安装、无需网络、无需服务器。
...
```
FlowLedger/
├── index.html        # 主应用（全部前端代码，约 4700 行）
