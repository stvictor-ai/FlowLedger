...
[在线体验](https://stvictor-ai.github.io/FlowLedger/) · [使用路径](#现在怎么用) · [核心功能](#核心功能) · [账号同步](#投记账号同步) · [自建部署](docs/deployment/aws.md) · [迭代路线](#迭代路线)
...
6. **同步数据**：可使用投记账号或 Private Gist 在设备间迁移账本，也可导入导出和恢复快照。
...
- **两种同步方式**：可选邀请制投记账号 + PostgreSQL，也保留用户自己的 Private Gist
...
- **投记账号同步**：邀请制邮箱账号、HttpOnly Cookie、PostgreSQL 持久化和 revision 并发保护
- 首次登录只比较数据，不自动覆盖；可明确选择合并、上传本机、下载服务器或暂不迁移
...
正式服务只公开 80/443，PostgreSQL 不映射到公网。API 启动时自动执行数据库迁移，可通过命令行生成限次、限时的邀请码。
...
自建版提供邀请制账号同步，同时保留 localStorage 本地副本：

1. 管理员在服务器生成邀请码，用户使用邮箱、至少 10 位密码和邀请码注册。
2. 登录凭证保存在 `HttpOnly` 安全 Cookie 中，不写入 localStorage。
3. 首次登录会比较本机与服务器的流水、持仓和金额摘要，不会自动上传或下载。
4. 用户可选择合并两边数据、以本机覆盖服务器、以服务器覆盖本机，或取消后继续本地使用。
5. 每次迁移前自动创建本地快照；服务器使用账本 `revision` 拒绝过期写入，避免两个设备静默互相覆盖。

账号服务是可选能力。未登录、服务器暂时不可用或退出账号后，本地记账、导入导出和 Gist 同步仍可继续使用。
