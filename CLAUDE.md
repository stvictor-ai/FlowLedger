...
## 项目概述
个人投资记账与资产追踪工具。以"统一净值视角"为核心：出入金记账提供成本与已实现盈亏，持仓按实时行情估值提供未实现盈亏，合并算出真实净值与真实收益率（真实总收益 = 已出金 + 当前持仓市值 − 已入金）。支持沪深市、美股、港股、加密货币、基金、黄金、期货等资产类型，覆盖记录管理、持仓追踪、盈亏统计、行为分析、复盘建议、数据备份与云同步。单文件 HTML 应用，可静态部署。
